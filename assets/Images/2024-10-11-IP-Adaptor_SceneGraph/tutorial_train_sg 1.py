...
--image_encoder_path ""     --output_dir /home/jskim/T2I/IP-Adapter/result     --logging_dir logs     \
--resolution 512     --learning_rate 0.000001     --weight_decay 0     --num_train_epochs 100     \
--train_batch_size 16     --dataloader_num_workers 32     --save_steps 2000     --mixed_precision no    \
...
def save_specific_parameters(model, optimizer, save_path, accelerator):
    
    state_dict = {
        'image_proj_model': model.image_proj_model.state_dict(),
        'adapter_modules': model.adapter_modules.state_dict(),
...
def main():
    ...
    sg_encoder.requires_grad_(False)
    ...
    for name in unet.attn_processors.keys():
        ...
        if cross_attention_dim is None:
            ...
            attn_procs[name] = IPAttnProcessor(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim, scale = 2)
    ...
    a = ip_adapter.named_parameters()
    for name, param in ip_adapter.named_parameters():
        if param.requires_grad:
            print(f"Layer: {name} is being trained.")
        else:
            pass
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(ip_adapter):
                ...
                # Gather the losses across all processes for logging (if distributed training).
        ...
        # 에포크가 끝난 후 에포크 단위 손실 기록
        if accelerator.is_main_process:
            average_epoch_loss = epoch_loss / num_batches
            writer.add_scalar("train/loss_epoch", average_epoch_loss, epoch)
